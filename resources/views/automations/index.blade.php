<x-app-layout>
    ...
    @push('scripts')
    {{-- Variable JS con la ruta de Laravel (necesita ser dinámica desde Blade) --}}
    <script>
        window.AF_STORE_URL = '{{ route('automations.store') }}';
    </script>

    {{-- Lógica estática: filtros, modal, SweetAlert → JS externo --}}
    <script src="{{ asset('js/app/automations-index.js') }}"></script>

    {{-- Sección dinámica: solo lo que necesita datos del servidor (errores, ?new, ?edit) --}}
    <script>
        @if ($errors->any())
